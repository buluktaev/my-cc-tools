...
if [ -n "$usage_data" ]; then
    ...
    week_used=$(echo "$usage_data" | jq -r '.seven_day.utilization // empty')
...
" 2>/dev/null || echo "")
    ...
    week_reset=$(echo "$usage_data" | python3 -c "
import sys, json
from datetime import datetime, timezone
data = json.load(sys.stdin)
reset = data.get('seven_day', {}).get('resets_at', '')
if reset:
...
# username
out="\033[1m$(whoami)\033[0m"

# Model
out="${out} • \033[36m${model}\033[0m"

# Usage limits
if [ -n "$five_h_left" ]; then
    five_color=$(usage_color "$five_h_left")
    if [ -n "$time_left" ]; then
        out="${out} • ${five_color}${time_left} ${five_h_left}%\033[0m"
    else
        out="${out} • ${five_color}5h ${five_h_left}%\033[0m"
    fi
fi
...
if [ -n "$week_left" ]; then
    ...
    if [ -n "$week_reset" ]; then
        out="${out} • ${week_color}W${week_reset} ${week_left}%\033[0m"
    else
        out="${out} • ${week_color}W ${week_left}%\033[0m"
    fi
fi

# Context
out="${out} • ${pct_color}${pct}%\033[0m"
